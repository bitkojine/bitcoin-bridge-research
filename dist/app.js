...
function renderAssessment(){
  ...
  document.querySelector('#assessment-result').innerHTML=`<div class="outcome ${cls}"><span>RESEARCH PRE-SCREEN</span><b>${outcome}</b></div><p class="detail-copy">${sat.length} evidenced · ${failed.length} gaps · ${unknown.length} unknown</p>${group('Blocking gaps',failed,true)}${group('Missing evidence',unknown,true)}${group('Evidenced',sat,false)}<div class="result-group"><h3>Limit</h3><div class="result-item">A qualified professional must validate the facts, applicability, and conclusion.</div></div>`;
}
