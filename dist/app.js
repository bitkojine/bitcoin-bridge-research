...
function renderCases(){
  ...
  root.innerHTML=case_studies.map(c=>{
    ...
    return `<article class="detail-panel" style="margin-bottom:24px">
      ...
      ${c.classification_analysis?`<h3>Is Bitcoin legally “crypto”?</h3><p><strong>${c.classification_analysis.question}</strong><br/>${c.classification_analysis.legal_answer}</p><p><strong>What Bitcoiners mean:</strong> ${c.classification_analysis.community_claim}</p><ul>${c.classification_analysis.distinctions.map(d=>`<li><strong>${d.level}:</strong> ${d.meaning}</li>`).join('')}</ul><p><strong>Practical effect:</strong> ${c.classification_analysis.practical_effect}</p>`:''}
